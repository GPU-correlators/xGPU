...
#if BLOCK_COMPLEX == 0
// Input Buffer: interleaved complex
// Shared Memory: block complex
...
#else
// Input Buffer: block complex
// Shared Memory: interleaved block complex
#define TWO_BY_TWO_COMPUTE(s)						\
 {float col1Xreal = input[s][2*tx];					\
  ...
  float row1Ximag = input[s][2*ty + 2*TILE_WIDTH];			\
  float row1Yreal = input[s][2*ty + 4*TILE_WIDTH];			\
  float row1Yimag = input[s][2*ty + 6*TILE_WIDTH];			\
  float row2Xreal = input[s][2*ty + 1];					\
  float row2Ximag = input[s][2*ty + 1 + 2*TILE_WIDTH];			\
  float row2Yreal = input[s][2*ty + 1 + 4*TILE_WIDTH];			\
  float row2Yimag = input[s][2*ty + 1 + 6*TILE_WIDTH];			\
  sum11XXreal += row1Xreal * col1Xreal;					\
  sum11XXreal += row1Ximag * col1Ximag;					\
  sum11XXimag += row1Ximag * col1Xreal;					\
  sum11XXimag -= row1Xreal * col1Ximag;					\
