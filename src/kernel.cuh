...
__device__ inline void findPosition(unsigned int &Col, unsigned int &Row, unsigned int &blockX, unsigned int &blockY) {
...
template<typename T>
__device__ inline void operator+=( T &a, const T b ) {
 T t = a;
...
__device__ inline void write2x2(unsigned int &Col, unsigned int &Row, float4 *matrix_real, float4 *matrix_imag, 
...
// device function to write out the matrix elements
__device__ inline void write2x2(unsigned int &Col, unsigned int &Row, int4 *matrix_real, int4 *matrix_imag, 
...
__global__ static void shared2x2(float4 *matrix_real, float4 *matrix_imag, const int Nstation, const int write)
{
...
#define BUFFER_DEPTH 4 // Kepler optimal setting
  ...
  findPosition(Col, Row, blockX, blockY);
...
#pragma unroll 1
  ...
  if (write) {
#endif
    write2x2(Col, Row, matrix_real, matrix_imag,
...
__global__ static void shared2x2(int4 *matrix_real, int4 *matrix_imag, const int Nstation, const int write)
{
